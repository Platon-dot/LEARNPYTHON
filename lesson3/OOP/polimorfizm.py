'''
Слово «полиморфизм» имеет греческую природу и означает «имеющий многие формы». 
На практике этот термин означает, что у разных объектов одинаковый метод может 
...
class Phone(Product): # далее внизу нужно описывать то, что отличает этот, к примеру, товар от текущих правил
    ...
    def get_memory_syze(self):
        # выводим размер паняти котрый хранится в байтах в Gb
        pass  # означает что этот метод будет реализована позже
...
class Sofa(Product): # далее внизу нужно описывать то, что отличает этот, к примеру, товар от текущих правил
    ...
    def get_color(self):
        return f'Цвет материала: {self.color}, Тип ткани: {self.texture}'
